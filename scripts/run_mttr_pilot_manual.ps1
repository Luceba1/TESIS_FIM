...
$evidencePath = Join-Path $evidenceDir "MTTR-PILOT-MANUAL.json"
...
Write-Host "Generando el MODIFIED del piloto manual..."

$actionStartedAt = [DateTimeOffset]::UtcNow

Add-Content -LiteralPath $fullPath `
    -Value "`nWatchDogs FIM - MTTR piloto manual - $($actionStartedAt.ToString('o'))" `
    -Encoding UTF8
...
$payload = @{
    ...
    source = "EXPERIMENT_CONTROLLED_MTTR_PILOT_MANUAL"
...
Write-Host ""
Write-Host "=========================================================="
Write-Host "EVENTO DETECTADO. COMENZÁ AHORA LA REVISIÓN MANUAL."
Write-Host "NO uses el frontend."
Write-Host "Usá la segunda PowerShell y seguí los comandos del protocolo manual."
Write-Host "Este script está esperando que marques el evento como REVIEWED."
Write-Host "=========================================================="
...
$evidence = [ordered]@{
    protocol = "MTTR_PILOT_MANUAL_V1"
    condition = "MANUAL_POWERSHELL_REVIEW"
...
Write-Host ""
Write-Host "=============================================="
Write-Host "FIN PILOTO MTTR MANUAL"
